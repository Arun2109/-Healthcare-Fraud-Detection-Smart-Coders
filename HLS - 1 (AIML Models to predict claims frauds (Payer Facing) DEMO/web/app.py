                                          #USE CASE NAME : HLS - 1 (AIML Models to predict claims frauds (Payer Facing) DEMO
...
def predict():
    ...
    if request.method=='POST':
        ...
        file_bene=request.files.get('benfi')
        
        temp_bene = pd.read_csv(file_bene)

        file_inp=request.files.get('inpatentt')
        temp_inp=pd.read_csv(file_inp)
        file_out=request.files.get('outpatient')
        temp_out=pd.read_csv(file_out)

        path_bene='C:\\Users\aruna\\Downloads\\healthcare-provider-fraud-detection-main\\healthcare-provider-fraud-detection-main\\'+file_bene.filename
        path_in='C:\\Users\\aruna\\Downloads\\healthcare-provider-fraud-detection-main\\healthcare-provider-fraud-detection-main\\'+file_inp.filename
        path_out='C:\\Users\\aruna\Downloads\\healthcare-provider-fraud-detection-main\\healthcare-provider-fraud-detection-main\\'+file_out.filename
        ...
        count=0
        #df=df['Prediction'].astype('int').tolist()
        count = 0
        for i in df['Prediction']:
                while(count<10):
                    if i == 1:
                        string='provider_id: '+id_[count]+' fraud'
                        count+=1
                        print(string)
                        break
                    else:
                        string='Provider_id: '+id_[count]+' Not fraud'
                        count+=1
                        print(string)
                        break
       

    return flask.render_template('result.html',df=df['Prediction'][0:10],id_=id_[0:10])
